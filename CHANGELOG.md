...
- 啟用認證時以帳號區分；未啟用認證時以來源電腦區分，頁面上會說明多人共用同一個對外位址時會看到彼此的作業。

### 新增 — 作業完成通知：站內鈴鐺 + Email / 通訊軟體 / Webhook

- 既有的「記錄轉送」（syslog / CEF / GELF）是給 SIEM 與管理員看的稽核軌跡 —— 它不會告訴「送出那份十幾分鐘轉檔的人」說他的檔案好了。這一版補上這一段：**通知送出者本人**。
- **站內鈴鐺**（側欄帳號旁）：未讀紅點、最近完成的作業清單、可一鍵標示為已讀。內容**直接由作業紀錄推導，不另開通知資料表** —— 另存一份就得雙寫，兩邊一旦不同步就會出現「通知說完成了、作業清單裡卻沒有」；作業被清掉時通知也自動跟著消失，不會留下點了沒有東西的項目。站內通知不需要任何設定就能用。
...
- 「同時處理數」是依**本機**的處理器與記憶體推算的，但呼叫外部 LLM 或遠端 GPU 辨識伺服器時，本機這一側只是上傳檔案後等回應 —— 真正的瓶頸在**對方那台機器**（一張顯示卡通常只常駐一個模型），本機的記憶體判斷擋不到。把同時處理數調高會意外變成對外部服務的壓力測試，結果是大家一起搶顯存、集體逾時。
- 因此外部呼叫另給一個上限，**預設 1**：一個一個過去、每個都在合理時間內完成，比全部一起去搶而全部變慢好。

### 新增 — 管理區「背景作業與併行度」：看得到排隊、可暫停、可調同時處理數

- 新頁面列出所有使用者的背景作業（送出者、工具、狀態、耗時），可取消個別作業，也可**暫停派工**（維護前讓手上的跑完但不再開新的）。暫停只影響尚未開始的作業，執行中的轉檔是獨立的子行程無法中途凍結 —— 頁面上照實說明。
...
- **避免記憶體耗盡是硬規則**：上限依實際可用記憶體推算並強制夾住（管理員填再大也無效）；派工前先估算再開一個需要多少記憶體，不夠就讓作業**留在佇列排隊**，不會硬開下去把機器打爆。記憶體讀取為容器感知，在 LXC / Docker 內取得的是容器自己的額度而非實體主機的數字。
...
- **修目錄鏡射帳號被靜默重新啟用（違反「鏡射 ≠ 啟用」不變量）**：排程「同步目錄使用者」更新既有列時，只更新顯示名稱，**不再連帶把 `enabled` 設回 1**。原本已去啟用的鏡射帳號只要在目錄端改名，下次同步就會被靜默恢復成可登入。
