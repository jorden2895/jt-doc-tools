...
### ⚠ 歷史紀錄的 id 直接從網址進來，沒有驗過格式

盤點「唯讀但會吐出檔案」的管理端點時抓到：

    GET /admin/history/{kind}/{hid}/file/{which}

`kind` 走白名單、`which` 走白名單，**只有 `hid` 沒驗**，而目錄是
`歷史根目錄 / hid`。Starlette 會把路徑參數裡的 `%2F` 解碼，所以 `hid`
拿得到 `../../..` —— 路徑就組到歷史目錄外面去了。

**沒有權限提升**：能讀到的檔案受白名單檔名限制（`original.pdf` / `preview.png`
／輸出檔名），而且這幾支端點是**稽核員專屬**的，連管理員都讀不到（裡面是
使用者的文件）。但 id 有固定格式（12 個十六進位字元）就該照格式驗 ——
**有 `safe_paths` 這層就是為了不要每次重新判斷「這次危不危險」**。

`get()` / `file()` / `delete()` 三支都先驗格式，不合法**一律當成找不到**
（端點自然回 404）。**不可以丟例外** —— 那會變成 500，而 500 的意思是
「伺服器壞了」，使用者送錯網址不該看到它。

> **我自己的測試第一版是錯的**：用管理員的 session 打那支端點，拿到 403
> ——那條測試根本沒走到處理函式。歷史檔案是稽核員專屬的，這也順手釘成一條
> 驗收（`tests/test_history_id_validation.py`，變異驗證 11 條裡 9 條會紅）。

### 測試計畫：新增 §4.9「唯讀但會吐出東西」的管理端點

§4.8 收的是會改狀態的那些，唯讀的原本刻意不列（讀的風險是洩漏，由 RBAC 守）。
但實算之後，19 支沒被點到名的唯讀端點裡有一類值得單獨寫：**會把檔案、或一段
要在客戶機器上執行的腳本交出去**，而路徑參數直接來自網址。三條共用判準
（路徑參數走格式白名單、歸屬與角色要與權限契約一致、產生的腳本不可夾帶未驗證
的值）＋ 六組端點。

順帶確認一件好消息：**`/admin` 底下會改狀態的端點，完整路徑已經 100% 出現在
計畫裡**（寫 0 支缺口）—— v1.15.30 補的那 79 支是完整的。

---

## [1.15.34] - 2026-09-13

### ⚠⚠ 設定檔寫到一半被中斷 —— 資料安靜地變成預設值

設定檔直接覆寫時，寫到一半斷電或行程被殺就留下一個**截斷的檔案**，而每一支
讀取端都是「剖析失敗就回預設值」：管理員一條一條建起來的東西安靜地不見，
畫面上看不出異常，也沒有任何錯誤訊息。

v1.15.20 為翻譯對照字典先補過這件事，當時記下「全站還有六支不是原子寫入」。
這次不靠那份清單、改用 AST 實算，結果**另外還有 12 支從來沒被列進去**，
其中三支的後果最重：

| 檔案 | 截斷之後會怎樣 |
|---|---|
| 認證設定 | 0 bytes 曾經等於「認證關閉」（讀取端已 fail-secure，這次把根因也堵掉）|
| **API Token** | 每張 token 消失，而且 `enforce` 退回 false ——**對外 API 的強制驗證無聲關掉** |
| 資產中繼資料 | 印章 / 簽名 / 浮水印的索引整份不見（檔案還在，清單空的）|

其餘九支：使用者工作區的檔案中繼資料與設定、上傳歸屬記錄、表單填寫 / 用印 /
浮水印歷史、學到的表單版型、乘車證明暫存清單與設定、電子發票設定、
送件檢核的案件與自家統編清單、OCR 引擎選擇、LLM 模型能力快取。

**做法是收成一支** `app/core/atomic_json.py`（同目錄暫存檔 → `fsync` →
`os.replace` → **再 fsync 目錄**），全站 30 處改走它。`os.replace` 的前置條件
每抄一次就多一次抄錯的機會 —— 實際上就抄錯過：有兩支把權限設在**換檔之後**，
於是含 token 的檔案有一瞬間是 0644；有一支拿唯讀的描述子去 fsync。

守門 `tests/test_settings_atomic_write.py` 逐檔掃 **AST 的 Call 節點**
（不掃字串也不掃註解 —— 這個專案被自己寫的註解騙過好幾次），例外要寫進清單
並說明原因，另有兩條驗「例外清單自己沒有過期」。

> **這件事不能靠清單。** 六支是人記下來的，實算之後發現真正的數字是 18 支。

### ⚠⚠ 轉好的檔案被丟掉，還叫使用者去裝 Java

`soffice` 的離開碼不可靠 —— 它會**一邊印無關的警告（`javaldx` 找不到 Java）
一邊正常轉完**，也可能在收尾階段才被中止。所以判準必須是「有沒有拿到一份
可用的檔案」。

這條規則 v1.14.46 就寫在 `convert_with_filter` 的註解裡，**但只有那一支照做**。
另外六支（轉 PDF / Draw 匯入 / .docx / .pptx / .odt / 純文字）都是先看
`returncode != 0` 就丟例外 —— 那六支涵蓋**辦公文件轉 PDF、PDF 轉文書檔、
PDF 轉簡報、Markdown 轉辦公文件、字數統計**這幾條主要路徑。

拿一支「產出檔照寫、離開碼回 1」的假 soffice 實測：

| | 修正前 | 修正後 |
|---|---|---|
| 六支舊的 | **全部丟例外**（轉好的檔案在暫存目錄裡被一起刪掉）| 採用那份產出 |
| `convert_with_filter` | 成功 | 成功 |

而使用者看到的訊息是「轉檔失敗：Warning: failed to launch javaldx」——
**指著一個完全無關的方向**，他會跑去裝 Java。

判準收成一支 `_require_output()`，六支都改走它。拿不到可用產出時才失敗，
而且三種情況分開講：空檔案（來源可能毀損）、被訊號中止（記憶體或同時轉太多，
跟檔案無關）、其他（附上 soffice 自己說的話）。

> **規則學到了卻只套用在一個地方**，跟這一版另一件事（原子寫入的六支 vs 18 支）
> 是同一個病。守門 `tests/test_office_convert_output_first.py` 用假 soffice
> 把「非零離開碼 ＋ 有可用產出」這個組合釘住，**並有反向對照**（真的沒有產出
> 時仍然要失敗）—— 少了反向那條，把檢查整段拿掉也會全綠。

### ⚠ 照 API 手冊呼叫卻壞：「測試 LLM 連線」不帶 body 會回 400

把 `API.md` 裡的 **84 條 curl 範例全部解析出來實際打一遍**
（`tools/api_doc_example_audit.py`，這次新增），抓到一條真的：
`/admin/api/llm/test-connection` 的範例沒有帶 body、文件也沒有參數表，
而端點 `await request.json()` 對空 body 丟例外 → 全域處理器回
**`400 Invalid JSON body`**。

照文件做的人看到的是「你送錯東西」，其實是**我們沒寫參數**。

修法兩邊都做：文件補上參數表與兩個範例（測存檔的設定 / 測還沒存的設定），
端點在沒有 body 時**退回已存檔的設定** —— 管理頁那顆按鈕送的是還沒存檔的
那組，所以 body 仍然優先。

> **對照層的守門管不到這一類。** 既有兩支測試驗的是「端點有沒有寫進文件」
> 與「幾支挑出來的參數對不對」，都不會把整條指令送出去。
> 68 條照文件呼叫成功、2 條是這台機器缺相依、其餘是文件本來就用佔位值
> （假統編、佔位 job id）或素材不足（空白 PDF 抽不到圖片）。


### 其他

- 測試計畫 §4.7 的「共 N 支」寫 267、實算 **265** —— 同一種病第 N 次
  （文件裡的數字沒有守門）。改成**從路由表實算比對**，並把新工具的五支端點
  從章節開頭挪進工具清單裡它該在的位置。
- API 手冊補上掃描拼合的 `422`（每一份都偵測不到內容區塊）—— 程式一直是這樣回，
  文件沒寫，串接的人只能自己試出來。**不改狀態碼**：它是刻意選的，而且改了會
  弄壞既有的呼叫端。
